...
function listele(secilecekNot) {
    ...
    for (const not of notlar) {
        const divOge = $("<a/>")
            ...
            .addClass(secilecekNot == not ? "active" : "");

        $("#lstNotlar").append(divOge); // tüm bu kodu appendTo("#lstNotlar") metodu ile tek satırda da yazabiliriz. 
    }
}
...
function formKabul(e) {
    ...
    if (seciliNot) {
        ...
        listele(yeniNot);
        seciliNot = yeniNot;
        // $("#lstNotlar>a").last().addClass("active"); // eğer listele metodu alfabetik sıralama yapsaydı bu yol doğru olmazdı. şu anki kodda çalışıyor sorun yok çünkü her eklenen listenin sonuna ekleniyor.
    }
    verileriSakla();
}
